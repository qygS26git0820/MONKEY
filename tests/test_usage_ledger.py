...
class AccumulationTest(unittest.TestCase):
    ...
    def test_known_values_accumulate(self):
        ledger = UsageLedger()
        ledger.record(input_tokens=100, output_tokens=20, cost=0.01)
        ledger.record(input_tokens=50, output_tokens=5, cost=0.005)
        self.assertEqual(150, ledger.input_tokens)
        self.assertEqual(25, ledger.output_tokens)
        self.assertAlmostEqual(0.015, ledger.cost)

    def test_fields_are_independent(self):
        ledger = UsageLedger()
        ledger.record(cost=0.02)
        self.assertEqual(0.02, ledger.cost)
        self.assertIsNone(ledger.input_tokens)
    ...
    def test_first_value_after_no_records_is_kept(self):
        ...
        ledger = UsageLedger()
        ledger.record(cost=0.01)
        self.assertEqual(0.01, ledger.cost)
...
class ExceededTest(unittest.TestCase):
    def test_no_caps_never_exceeds(self):
        ledger = UsageLedger()
        ledger.record(input_tokens=10**9, output_tokens=10**9, cost=999.0)
        self.assertFalse(ledger.exceeded(budgets()))

    def test_unknown_never_exceeds_even_with_a_cap(self):
        ledger = UsageLedger()
        self.assertFalse(ledger.exceeded(budgets(max_cost_cny=0.5)))

    def test_cost_over_cap_exceeds(self):
        ledger = UsageLedger()
        ledger.record(cost=0.51)
        self.assertTrue(ledger.exceeded(budgets(max_cost_cny=0.5)))

    def test_cost_exactly_at_cap_does_not_exceed(self):
        ledger = UsageLedger()
        ledger.record(cost=0.5)
        self.assertFalse(ledger.exceeded(budgets(max_cost_cny=0.5)))
